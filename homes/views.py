...
def fileRequest(request):
    if request.method == "POST":
        ...
        return StreamingHttpResponse(gen(cam, slow_fps=True), content_type="multipart/x-mixed-replace;boundary=frame")
...
class VideoCamera(object):
    ...
    def get_frame(self):
        ...
        if self.file_path is None:
            ...
            new_height = image.shape[1]//2
        else:
            new_width = image.shape[0]//4
            new_height = image.shape[1]//4
...
def gen(camera, slow_fps=False):
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while True:
            ...
            # cv2.imshow('Sign Language Translation', image_org)
